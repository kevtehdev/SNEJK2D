...
void Game_Init(Game *_Game)
{
    ...
    _Game->eventDispatcher = malloc(sizeof(EventDispatcher));
    if (_Game->eventDispatcher)
    {
        EventSystem_Init(_Game->eventDispatcher);
    }
}
    ...
    if (_Game->gameMode == MODE_POWERUP)
    {
        ...
        for (int p = 0; p < POWERUP_MODE_COUNT; p++)
        {
            ...
            if (valid)
            {
                ...
                if (_Game->eventDispatcher)
                {
                    Event spawnedEvent = Event_CreatePowerUpSpawned(spawnType, spawnPos);
                    EventSystem_Emit(_Game->eventDispatcher, &spawnedEvent);
                }
            }
        }
    }
...
void Game_Update(Game *_Game)
{
    ...
    if (_Game->powerUpStatus.speedBoostActive && currentTime >= _Game->powerUpStatus.speedBoostEndTime)
    {
        ...
        if (_Game->eventDispatcher)
        {
            Event expiredEvent = Event_CreatePowerUpExpired(POWERUP_SPEED_BOOST);
            EventSystem_Emit(_Game->eventDispatcher, &expiredEvent);
        }
    }
    ...
    if (_Game->powerUpStatus.scoreMultiplierActive && currentTime >= _Game->powerUpStatus.scoreMultiplierEndTime)
    {
        ...
        if (_Game->eventDispatcher)
        {
            Event expiredEvent = Event_CreatePowerUpExpired(POWERUP_SCORE_MULTIPLIER);
            EventSystem_Emit(_Game->eventDispatcher, &expiredEvent);
        }
    }
    ...
    if (currentTime - _Game->lastMoveTime >= _Game->currentSpeed)
    {
        ...
        if (wouldHitWall || wouldHitSelf)
        {
            ...
            // Emit snake died event
            if (_Game->eventDispatcher)
            {
                Event snakeDiedEvent = Event_CreateSnakeDied();
                EventSystem_Emit(_Game->eventDispatcher, &snakeDiedEvent);
            }
        ...
        if (Game_CheckFoodCollision(_Game))
        {
            ...
            int oldScore = _Game->snake.score;
            int points = (int)(basePoints * totalMultiplier);
            _Game->snake.score += points;

            // Emit food eaten and score changed events
            if (_Game->eventDispatcher)
            {
                Event foodEatenEvent = Event_CreateFoodEaten();
                EventSystem_Emit(_Game->eventDispatcher, &foodEatenEvent);

                Event scoreChangedEvent = Event_CreateScoreChanged(_Game->snake.score, oldScore);
                EventSystem_Emit(_Game->eventDispatcher, &scoreChangedEvent);
            }
            ...
            if (_Game->gameMode == MODE_POWERUP)
            {
                ...
                while (_Game->xp >= _Game->xpToNextLevel)
                {
                    ...
                    if (_Game->eventDispatcher)
                    {
                        Event levelUpEvent = Event_CreateLevelUp(_Game->level, _Game->xp, _Game->xpToNextLevel);
                        EventSystem_Emit(_Game->eventDispatcher, &levelUpEvent);
                    }
                }
            }
        ...
        if (_Game->gameMode == MODE_POWERUP)
        {
            ...
            for (int i = 0; i < MAX_POWERUPS; i++)
            {
                    ...
                    head.y == _Game->powerUps[i].position.y)
                {
                    ...
                    Position powerUpPos = _Game->powerUps[i].position;
                    _Game->powerUps[i].active = false; // Consume power-up

                    // Emit power-up collected event
                    if (_Game->eventDispatcher)
                    {
                        Event powerUpEvent = Event_CreatePowerUpCollected(type, powerUpPos);
                        EventSystem_Emit(_Game->eventDispatcher, &powerUpEvent);
                    }
