...
// muahaha your mpapi has been vibecoded to be cross-compatible with windows
...
static int read_line(socket_t fd, char **out_line);
...
static int ensure_connected(mpapi *api) {
    if (!api) return MPAPI_ERR_ARGUMENT;
    if (api->sockfd != INVALID_FD) return MPAPI_OK;
...
static int send_json_line(mpapi *api, json_t *obj) {
...
#ifdef DEV_BUILD
    printf("[MpApi] Sending JSON: %s\n", text); // Debug print
#endif

    size_t len = strlen(text);
    int fd = api->sockfd;

    // Combine JSON + newline into one buffer
    ...
    // send_all returns 0 on success, -1 on error
    if (send_all(fd, buf, total_len) != 0) {
#ifdef _WIN32
        int err = WSAGetLastError();
        printf("[MpApi] send_all failed with WSA error: %d\n", err);
#else
        perror("[MpApi] send_all failed");
#endif
        rc = MPAPI_ERR_IO;
    }
